...
// MD5 basic transformation transform.
...
static void MD5Encode(unsigned char *output, const uint32_t *input, unsigned int len);
...
static void MD5Transform(uint32_t state[4], const unsigned char block[64]) {
    ...
    II (a, b, c, d, x[11], 6, 0xf7537e82); /* 61 */
    II (d, a, b, c, x[ 2], 10, 0xbd3af235); /* 62 */
    II (c, d, a, b, x[ 9], 15, 0x2ad7d2bb); /* 63 */
    II (b, c, d, a, x[ 4], 21, 0xeb86d391); /* 64 */
...
static void computeMD5(const char *input, char *outputHash) {
    MD5_CTX context;
    unsigned char digest[16];
    int i;

    MD5Init(&context);
    MD5Update(&context, (const unsigned char *)input, (unsigned int)strlen(input));
